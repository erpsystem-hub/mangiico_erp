...
import {
  buildMaterialCategoryLevelBadgeConfig,
  materialCategoryTrangThaiBadgeConfig,
} from '../utils/material-category-badges';
...
const MaterialCategoryDetail: React.FC<Props> = ({
  data,
  allCategories,
  onClose,
  onEdit,
  ...
  return (
    <GenericDrawer
      title={txt('materialCategory.detail.title')}
      icon={<Layers size={18} />}
      onClose={onClose}
      footer={renderFooter}
      footerCompact
    ...
    >
      <div className="space-y-5">
        ...
        <DetailSection title={txt('materialCategory.detail.basicInfo')} icon={<Layers size={14} />} variant="primary">
          <DetailFieldGrid>
            <DetailField label={txt('materialCategory.name')} value={data.ten_danh_muc} icon={<Layers size={12} />} />
            <DetailField
            ...
            <DetailField
              ...
              value={parentDept ? parentDept.ten_danh_muc : txt('materialCategory.detail.noParent')}
              icon={<Folder size={12} />}
              emptyText={txt('materialCategory.detail.noParent')}
            />
            <DetailField
              label={txt('materialCategory.detail.level')}
              value={
                <EnumBadge
                  shape="rounded"
                  value={data.cap_do}
                  config={levelBadgeConfig}
                  fallbackLabel={txt('materialCategory.levelBadge', { level: data.cap_do })}
                />
              }
              icon={<Layers size={12} />}
            />
            <DetailField label={txt('materialCategory.detail.order')} value={String(data.thu_tu)} icon={<ArrowUpFromLine size={12} />} />
            <DetailField
        ...
        <DetailSection title={txt('materialCategory.detail.systemInfo')} icon={<Clock size={14} />} variant="primary">
          <DetailFieldGrid>
            <DetailField
              label={txt('materialCategory.detail.createdAt')}
              value={formatDateTimeShort(data.tg_tao)}
              icon={<Calendar size={12} />}
            />
            <DetailField
              label={txt('materialCategory.detail.updated')}
        ...
        {isRootLevel ? (
          <DetailSection
            title={txt('materialCategory.detail.childrenSection')}
            icon={<Layers size={14} />}
            variant="primary"
            headerRight={
              <>
                <span className="inline-flex items-center rounded-full border border-primary/20 bg-primary/10 px-2.5 py-1 text-xs font-medium tabular-nums text-primary">
                  {children.length} {txt('materialCategory.footerRecords')}
                </span>
                {onAddChild && canCreate ? (
                  ...
                  >
                    ...
                    {txt('materialCategory.detail.addChild')}
                  </Button>
                ) : null}
              </>
            }
          >
            {children.length === 0 ? (
              <EmptyState
                title={txt('materialCategory.detail.noChildren')}
                description={txt('materialCategory.detail.noChildrenHint')}
              ...
              <EmbeddedChildDataGrid<MaterialCategory>
                rows={children}
                getRowKey={(child) => child.id}
                labelColumn={{
                  header: txt('materialCategory.name'),
                ...
                columns={[
                  {
                    id: 'desc',
                    header: txt('materialCategory.store.descCol'),
                ...
                actionsColumn={{
                  ...
                  renderCell: (child) => (
                    <MaterialCategoryTableRowActions
                      compact
                      item={child}
