...
interface Props {
  ...
  onView?: (item: MaterialCategory) => void;
}

const treeOptions = {
  getId: (d: MaterialCategory) => d.id,
  getParentId: (d: MaterialCategory) => d.cha_id,
  getOrder: (d: MaterialCategory) => d.thu_tu,
  includeOrphans: true as const,
};

const MaterialCategoryList: React.FC<Props> = ({
  data,
  allCategories,
  columns,
  selectedIds,
  ...
  const { canEdit, canDelete } = useResourcePermissions('materialCategories');
  const { menuOpenId, setMenuOpenId } = useRowMenuOpenState();
  const { filters, setFilter, sort, setSort } = useMaterialCategoryStore();
        ...
        case 'ten_danh_muc':
          return (
            <ColumnHeaderFilter
              ...
              ariaLabel={txt('materialCategory.toolbar.rootCategory')}
  ...
  if (isLoading) {
    return (
      <ListPageSkeleton
        loadingText={txt('materialCategory.loading')}
  ...
  if (allCategories.length === 0) {
    return (
      <div className="flex-1 min-h-0 flex items-center justify-center p-4">
        <EmptyState
          title={txt('materialCategory.empty')}
          description={txt('materialCategory.emptyHint')}
  ...
  const renderCell = (dept: MaterialCategory, col: ColumnConfig) => {
        ...
        return (
          <td key={col.id} className="px-6 py-1.5" style={getColumnCellStyle(col)}>
            <EnumBadge
              ...
              fallbackLabel={txt('materialCategory.levelBadge', { level: dept.cap_do })}
            />
          </td>
        );
      case 'trang_thai':
  ...
  const renderMobileCard = (dept: MaterialCategory, isSelected: boolean) => {
      ...
      <MobileListCard
        ...
        metaLine={(
          <p className="text-xs text-muted-foreground">
            {txt('materialCategory.detail.level')} {dept.cap_do}
          </p>
        )}
        subheader={
          hasExtra ? (
            <div className="grid grid-cols-2 gap-2 rounded-lg bg-muted/30 px-3 py-2 text-body-sm">
              {dept.cap_do != null && (
                <div>
                  <p className="mb-0.5 text-muted-foreground">{txt('materialCategory.detail.level')}</p>
                  <EnumBadge
                    shape="rounded"
                    value={dept.cap_do}
                    config={levelBadgeConfig}
              ...
              {dept.mo_ta ? (
                <div className={dept.cap_do != null ? '' : 'col-span-2'}>
                  <p className="mb-0.5 text-muted-foreground">{txt('materialCategory.detail.description')}</p>
        ...
        footerStart={(
          <div className="flex min-w-0 flex-1 items-center gap-2">
            ...
            <span className="text-xs text-muted-foreground">
              {txt('materialCategory.detail.order')}: {dept.thu_tu}
            </span>
          </div>
        )}
        footerEnd={
    ...
    <div className="flex flex-col h-full min-h-0 bg-card overflow-hidden">
      ...
      <div className="flex-1 min-h-0 flex flex-col overflow-hidden">
        ...
        <div className="hidden md:flex flex-1 min-h-0 flex-col overflow-hidden">
          <HierarchyTable<MaterialCategory>
            data={paginatedData}
      ...
      <div className="shrink-0 border-t border-border bg-muted/30">
        <TablePaginationFooter
          ...
          recordsLabel={txt('materialCategory.footerRecords')}
        />
      </div>
    </div>
  );
