...
import ProductCatalogToolbar from './components/danh-sach-hang-hoa-toolbar';
import ProductCatalogTable from './components/danh-sach-hang-hoa-table';
import {
  useProductCatalogList,
  useDeleteProductCatalogItems,
  useUpdateProductCatalogStatus,
...
const ProductCatalogPage: React.FC = () => {
  ...
  const canView = useCan('view', 'productCatalog');
  const { isInitializing } = useAppSessionReady();
  const { canCreate, canEdit, canDelete } = useResourcePermissions('productCatalog');
  ...
  const [viewingItem, setViewingItem] = useState<ProductCatalogItem | null>(null);
  const [formOrigin, setFormOrigin] = useState<FormOrigin>('list');

  const { searchTerm, filters, resetState, clearSelection, selectedIds } = useProductCatalogStore();

  useEffect(() => {
    if (!user || canView || didRedirect.current) return;
    didRedirect.current = true;
    toast.error(txt('productCatalog.noViewPermission'));
    navigate('/san-xuat', { replace: true });
  }, [user, canView, navigate]);

  const { data: categories = [] } = useProductCategories({ enabled: canView });
  const { data: items = [], isLoading, isError, refetch } = useProductCatalogList({
    enabled: canView,
  });
  ...
  const filterFn = useCallback(
    (item: ProductCatalogItem, term: string, f: typeof filters) =>
      matchesProductCatalogFilters(item, categories, term, f),
    [categories],
  );
  ...
  const handleView = useCallback(
    (item: ProductCatalogItem) => {
      queryClient.setQueryData(queryKeys.productCatalog.detail(item.id), item);
      setViewingItem(item);
    },
    [queryClient],
  );
  ...
  const handleDelete = (id: string) => {
    if (!canDelete) return;
    confirm({
      title: txt('productCatalog.deleteTitle'),
      message: txt('productCatalog.deleteMessage'),
      variant: 'danger',
  ...
  const handleStatusChange = (item: ProductCatalogItem) => {
    if (!canEdit) return;
    const newStatus: TrangThaiHoatDong =
      item.trang_thai === 'Đang hoạt động' ? 'Ngừng hoạt động' : 'Đang hoạt động';
    confirm({
      title: txt('productCatalog.statusChangeTitle'),
      message: `${txt('productCatalog.statusChangeMessage')} ${newStatus}?`,
  ...
  const handleDeleteMany = (ids: string[]) => {
    ...
    confirm({
      title: txt('productCatalog.bulkDeleteTitle'),
      message: txt('productCatalog.bulkDeleteMessage', { count: ids.length }),
      variant: 'danger',
      confirmText: CONFIRM_DELETE(),
    ...
    <div className="flex flex-col h-page relative">
      <div className="flex-1 min-h-0 flex flex-col mt-1.5 rounded-xl border border-border bg-card shadow-sm overflow-hidden">
        ...
        <div className="flex-1 min-h-0 flex flex-col">
          {isError ? (
            <ErrorState
              title={txt('productCatalog.listLoadErrorTitle')}
              message={txt('productCatalog.listLoadErrorHint')}
              onRetry={() => refetch()}
              primaryButtons
      ...
      <AnimatePresence>
        {showForm && (
          <Suspense fallback={<DrawerLazyFallback />}>
            <ProductCatalogForm initialData={editingItem} onClose={handleCloseForm} />
          </Suspense>
        )}
      </AnimatePresence>
