...
import { countBomColumnSearchActive } from '../utils/column-search';
...
const BomToolbar: React.FC<Props> = ({
  categories,
  items,
  onAdd,
  onDeleteMany,
  ...
  const { canCreate, canDelete, canEdit } = useResourcePermissions('bom');
  ...
  const activeFilterCount = useMemo(() => {
    const colN = countBomColumnSearchActive(filters.columnSearch);
    return (
      (searchTerm ? 1 : 0) +
      colN +
