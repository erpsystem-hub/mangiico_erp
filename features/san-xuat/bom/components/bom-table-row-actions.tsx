...
export interface BomTableRowActionsProps {
  item: BomItem;
...
export function BomTableRowActions({
  item,
  menuOpenId,
  onMenuOpenChange,
  onEdit,
  ...
  onStatusChange,
  compact = false,
}: BomTableRowActionsProps) {
  const close = () => onMenuOpenChange(null);
  const canEdit = useCan('edit', 'bom');
  const canDelete = useCan('delete', 'bom');

  const toggleLabel =
    item.trang_thai === 'Đang hoạt động'
      ? txt('bom.detail.deactivate')
      : txt('bom.detail.activate');
