...
import type { MatrixCategoryFilters } from '../utils/matrix-category-filter';
...
interface Props {
  ...
  onFilterChange: <K extends keyof MatrixCategoryFilters>(
    key: K,
    value: MatrixCategoryFilters[K],
  ) => void;
  activeFilterCount: number;
  onClearAllFilters: () => void;
}

const CategoryLinkMatrixToolbar: React.FC<Props> = ({
  categories,
  searchTerm,
  onSearchChange,
  filters,
  ...
  const filterGroups = useMemo(
    () => [
      {
        key: 'id_danh_muc_goc',
        label: txt('productCategory.toolbar.rootCategory'),
        icon: Building2,
    ...
    () => (
      <div className="flex flex-wrap items-center gap-2 min-w-0">
        <FilterChipMultiSelect
          ...
          placeholder={txt('productCategory.toolbar.rootCategory')}
          icon={Building2}
  ...
  return (
    <GenericToolbar
      ...
      onClearSelection={() => {}}
      searchPlaceholder={txt('productCategory.matrix.searchPlaceholder')}
      filters={filtersSlot}
...
export default CategoryLinkMatrixToolbar;
