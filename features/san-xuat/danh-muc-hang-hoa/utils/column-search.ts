...
export const PRODUCT_CATEGORY_COLUMN_IDS_WITH_MULTISELECT = [
  'trang_thai',
  'ten_danh_muc',
] as const;
...
export function countProductCategoryColumnSearchActive(
  columnSearch: Record<string, string> | undefined,
): number {
  if (!columnSearch) return 0;
  const skip = PRODUCT_CATEGORY_COLUMN_IDS_WITH_MULTISELECT as readonly string[];
  let n = 0;
...
export function productCategoryMatchesColumnSearch(
  item: ProductCategory,
  columnSearch: Record<string, string> | undefined,
  parentName: string,
): boolean {
