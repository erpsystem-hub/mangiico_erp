...
import { countMaterialCatalogColumnSearchActive } from '../utils/column-search';
...
const MaterialCatalogToolbar: React.FC<Props> = ({
  categories,
  items,
  onAdd,
  onDeleteMany,
  ...
  const { canCreate, canDelete, canEdit } = useResourcePermissions('materialCatalog');
  ...
  const activeFilterCount = useMemo(() => {
    const colN = countMaterialCatalogColumnSearchActive(filters.columnSearch);
    return (
      (searchTerm ? 1 : 0) +
      colN +
...
export default MaterialCatalogToolbar;
