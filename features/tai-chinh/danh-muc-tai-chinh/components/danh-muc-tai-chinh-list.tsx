...
interface Props {
  ...
  onView?: (item: FinanceCategory) => void;
}

const treeOptions = {
  getId: (d: FinanceCategory) => d.id,
  getParentId: (d: FinanceCategory) => d.cha_id,
  getOrder: (d: FinanceCategory) => d.thu_tu,
  includeOrphans: true as const,
};

const FinanceCategoryList: React.FC<Props> = ({
  data,
  allCategories,
  columns,
  selectedIds,
  ...
  const { canEdit, canDelete } = useResourcePermissions('financeCategories');
  const { menuOpenId, setMenuOpenId } = useRowMenuOpenState();
  const { filters, setFilter, sort, setSort } = useFinanceCategoryStore();
    ...
    () => [
      {
        ...
        value: 'Thu',
        count: allCategories.filter((d) => d.loai === 'Thu').length,
      },
      {
        label: txt('financeCategory.typeChi'),
        value: 'Chi',
        count: allCategories.filter((d) => d.loai === 'Chi').length,
      },
    ],
    [allCategories],
  );
  ...
  const renderColumnHeaderAccessory = useCallback(
        ...
        case 'ten_danh_muc':
          return (
            <ColumnHeaderFilter
              ...
              ariaLabel={txt('financeCategory.toolbar.rootCategory')}
        ...
        case 'loai':
          return (
            <ColumnHeaderFilter
              ...
              ariaLabel={txt('financeCategory.type')}
              sortColumnId="loai"
              sort={sort}
              setSort={setSort}
            />
    ...
    [filters, setFilter, sort, setSort, phongOptionsWithCount, statusOptions, loaiOptions]
  );
  ...
  if (isLoading) {
    return (
      <ListPageSkeleton
        loadingText={txt('financeCategory.loading')}
  ...
  if (allCategories.length === 0) {
    return (
      <div className="flex-1 min-h-0 flex items-center justify-center p-4">
        <EmptyState
          title={txt('financeCategory.empty')}
          description={txt('financeCategory.emptyHint')}
  ...
  const renderCell = (dept: FinanceCategory, col: ColumnConfig) => {
    ...
    switch (col.id) {
      ...
      case 'loai':
        return (
          <td key={col.id} className="px-6 py-1.5" style={getColumnCellStyle(col)}>
            <EnumBadge shape="pill" value={dept.loai} config={loaiBadgeConfig} />
          </td>
        );
        ...
        return (
          <td key={col.id} className="px-6 py-1.5" style={getColumnCellStyle(col)}>
            <EnumBadge
              ...
              fallbackLabel={txt('financeCategory.levelBadge', { level: dept.cap_do })}
            />
          </td>
        );
      case 'trang_thai':
  ...
  const renderMobileCard = (dept: FinanceCategory, isSelected: boolean) => {
      ...
      <MobileListCard
        ...
        metaLine={(
          <p className="text-xs text-muted-foreground">
            {txt('financeCategory.detail.level')} {dept.cap_do}
          </p>
        )}
        subheader={
          hasExtra ? (
            <div className="grid grid-cols-2 gap-2 rounded-lg bg-muted/30 px-3 py-2 text-body-sm">
              {dept.cap_do != null && (
                <div>
                  <p className="mb-0.5 text-muted-foreground">{txt('financeCategory.detail.level')}</p>
                  <EnumBadge
                    shape="rounded"
                    value={dept.cap_do}
                    config={levelBadgeConfig}
              ...
              {dept.mo_ta ? (
                <div className={dept.cap_do != null ? '' : 'col-span-2'}>
                  <p className="mb-0.5 text-muted-foreground">{txt('financeCategory.detail.description')}</p>
        ...
        footerStart={(
          <div className="flex min-w-0 flex-1 items-center gap-2">
            ...
            <span className="text-xs text-muted-foreground">
              {txt('financeCategory.detail.order')}: {dept.thu_tu}
            </span>
          </div>
        )}
        footerEnd={
    ...
    <div className="flex flex-col h-full min-h-0 bg-card overflow-hidden">
      ...
      <div className="flex-1 min-h-0 flex flex-col overflow-hidden">
        ...
        <div className="hidden md:flex flex-1 min-h-0 flex-col overflow-hidden">
          <HierarchyTable<FinanceCategory>
            data={paginatedData}
      ...
      <div className="shrink-0 border-t border-border bg-muted/30">
        <TablePaginationFooter
          ...
          recordsLabel={txt('financeCategory.footerRecords')}
        />
      </div>
    </div>
  );
