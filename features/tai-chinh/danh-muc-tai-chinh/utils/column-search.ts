...
export const FINANCE_CATEGORY_COLUMN_IDS_WITH_MULTISELECT = [
  'trang_thai',
  'ten_danh_muc',
  'loai',
] as const;
...
export function countFinanceCategoryColumnSearchActive(
  columnSearch: Record<string, string> | undefined,
): number {
  if (!columnSearch) return 0;
  const skip = FINANCE_CATEGORY_COLUMN_IDS_WITH_MULTISELECT as readonly string[];
  let n = 0;
...
export function financeCategoryMatchesColumnSearch(
  item: FinanceCategory,
  columnSearch: Record<string, string> | undefined,
  parentName: string,
): boolean {
