...
const BranchToolbar: React.FC<Props> = ({
  ...
  const { canCreate, canImport, canExport, canDelete, canEdit } = useResourcePermissions('branches');
  ...
  const activeFilterCount = useMemo(() => {
    ...
    return (searchTerm ? 1 : 0) + columnSearchN + (filters.status.length > 0 ? 1 : 0);
  }, [searchTerm, filters]);
  ...
  const filtersSlot = useMemo(
    ...
    [statusOptions, filters.status, setFilter],
  );
  ...
  const filterGroups = useMemo(
    ...
    [filters.status, setFilter, statusOptions],
  );
