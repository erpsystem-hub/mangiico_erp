...
interface Props {
  ...
  onView?: (item: PartnerCategory) => void;
}
...
const PartnerCategoryList: React.FC<Props> = ({
  categoryResource,
  data,
  allCategories,
  columns,
  ...
  const { canEdit, canDelete } = useResourcePermissions(categoryResource);
  const { menuOpenId, setMenuOpenId } = useRowMenuOpenState();
  const { filters, setFilter, sort, setSort } = usePartnerCategoryStore();
        ...
        case 'ten_danh_muc':
          return (
            <ColumnHeaderFilter
              ...
              ariaLabel={txt('partnerCategory.toolbar.rootCategory')}
  ...
  if (isLoading) {
    return (
      <ListPageSkeleton
        loadingText={txt('partnerCategory.loading')}
    ...
    return (
      <div className="flex-1 min-h-0 flex items-center justify-center p-4">
        <EmptyState
          ...
          description={txt('partnerCategory.emptyHint')}
  ...
  const renderCell = (dept: PartnerCategory, col: ColumnConfig) => {
        ...
        return (
          <td key={col.id} className="px-6 py-1.5" style={getColumnCellStyle(col)}>
            <EnumBadge
              ...
              fallbackLabel={txt('partnerCategory.levelBadge', { level: dept.cap_do })}
            />
          </td>
        );
      case 'trang_thai':
  ...
  const renderMobileCard = (dept: PartnerCategory, isSelected: boolean) => {
      ...
      <MobileListCard
        ...
        metaLine={(
          <p className="text-xs text-muted-foreground">
            {txt('partnerCategory.detail.level')} {dept.cap_do}
          </p>
        )}
        subheader={
          hasExtra ? (
            <div className="grid grid-cols-2 gap-2 rounded-lg bg-muted/30 px-3 py-2 text-body-sm">
              {dept.cap_do != null && (
                <div>
                  <p className="mb-0.5 text-muted-foreground">{txt('partnerCategory.detail.level')}</p>
                  <EnumBadge
                    shape="rounded"
                    value={dept.cap_do}
                    config={levelBadgeConfig}
              ...
              {dept.mo_ta ? (
                <div className={dept.cap_do != null ? '' : 'col-span-2'}>
                  <p className="mb-0.5 text-muted-foreground">{txt('partnerCategory.detail.description')}</p>
        ...
        footerStart={(
          <div className="flex min-w-0 flex-1 items-center gap-2">
            ...
            <span className="text-xs text-muted-foreground">
              {txt('partnerCategory.detail.order')}: {dept.thu_tu}
            </span>
          </div>
        )}
        footerEnd={
    ...
    <div className="flex flex-col h-full min-h-0 bg-card overflow-hidden">
      ...
      <div className="flex-1 min-h-0 flex flex-col overflow-hidden">
        ...
        <div className="hidden md:flex flex-1 min-h-0 flex-col overflow-hidden">
          <HierarchyTable<PartnerCategory>
            data={paginatedData}
      ...
      <div className="shrink-0 border-t border-border bg-muted/30">
        <TablePaginationFooter
          ...
          recordsLabel={txt('partnerCategory.footerRecords')}
        />
      </div>
    </div>
  );
