...
/** Phiếu kiểm kê — module thiết bị có thể chưa có trong repo; giữ tối thiểu field dùng khi in. */
...
export const printAuditTicket = (ticket: AuditTicket, companyInfo?: CompanyPrintInfo) => {
    const rows = ticket.chi_tiet.map((item: AuditTicketChiTiet, index: number) => `
        <tr>
...
export const printMaintenanceTicket = (ticket: MaintenanceTicket, companyInfo?: CompanyPrintInfo) => {
    const sparePartsRows = ticket.linh_kien_su_dung?.map((item: MaintenanceSparePart, index: number) => `
        <tr>
            <td class="text-center">${index + 1}</td>
