-- Chạy trực tiếp trên Supabase SQL Editor (Dashboard → SQL → New query).
-- Bỏ cột cấp quản lý khỏi chức vụ + cập nhật RPC danh sách nhân viên.
-- Yêu cầu: đã có bảng var_chi_nhanh / var_nhan_vien_chi_nhanh (migration var_chi_nhanh).
-- Nếu chưa có chi nhánh: chỉ chạy phần DROP COLUMN, rồi sửa RPC get_nhan_vien_page thủ công.
