-- Script tạo BOM theo dòng đơn hàng (chạy trên Supabase SQL Editor)
-- Đồng bộ migration 20260712900000_kd_don_hang_chi_tiet_bom.sql
BEGIN;
...
BEGIN
  SELECT ct.danh_muc_id, ct.so_luong INTO v_danh_muc_id, v_line_qty
  FROM public.kd_don_hang_chi_tiet ct WHERE ct.id = p_don_hang_chi_tiet_id;
  IF v_danh_muc_id IS NULL THEN
    RAISE EXCEPTION 'kd_generate_don_hang_chi_tiet_bom: dòng đơn % không tồn tại', p_don_hang_chi_tiet_id;
  END IF;
  SELECT count(*)::int INTO v_existing FROM public.kd_don_hang_chi_tiet_bom
  ...
  END IF;
  SELECT count(*)::int INTO v_template_count FROM public.sx_bom b
  WHERE b.danh_muc_id = v_danh_muc_id AND b.trang_thai = 'Đang hoạt động';
  ...
  END IF;
  IF p_replace THEN
    DELETE FROM public.kd_don_hang_chi_tiet_bom WHERE don_hang_chi_tiet_id = p_don_hang_chi_tiet_id;
  END IF;
  INSERT INTO public.kd_don_hang_chi_tiet_bom (
    don_hang_chi_tiet_id, nguyen_lieu_id, bom_mau_id,
    so_luong_dinh_muc, so_luong_tong, don_vi_tinh, ghi_chu, thu_tu
  )
  SELECT p_don_hang_chi_tiet_id, b.nguyen_lieu_id, b.id,
         b.so_luong, b.so_luong * v_line_qty, b.don_vi_tinh, b.ghi_chu, b.thu_tu
